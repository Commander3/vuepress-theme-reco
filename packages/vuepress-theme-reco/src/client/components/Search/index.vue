...
import { pathToFile, escapeRegExp, LRUCache } from './utils'
// TODO using the theme data
...
const disableDetailedView = computed(() => {
  return (
    ...
    //   theme.value.search.options?.detai  ledView === false)
    true
  )
})
    ...
    <div ref="el" role="button" :aria-owns="results?.length ? 'localsearch-list' : undefined" aria-expanded="true"
      ...
      <div class="shell">
        <form class="search-bar" @pointerup="onSearchBarClick($event)" @submit.prevent="">
          ...
          <input ref="searchInput" v-model="filterText"
        ...
        <ul ref="resultsEl" :id="results?.length ? 'localsearch-list' : undefined"
          ...
          :aria-labelledby="results?.length ? 'localsearch-label' : undefined" class="results" @mousemove="onMouseMove">
          <li v-for="(p, index) in results" :key="p.id" :id="'localsearch-item-' + index"
            :aria-selected="selectedIndex === index ? 'true' : 'false'" role="option">
            <a :href="p.id" class="result" :class="{
              selected: selectedIndex === index
            }" :aria-label="[...p.titles, p.title].join(' > ')"
              @mouseenter="!disableMouseOver && (selectedIndex = index)" @focusin="selectedIndex = index"
              @click="$emit('close')" :data-index="index">
              <div>
                <div class="titles">
